...
class AddAdditionMetricsToHistoricals extends Migration
{
    ...
    public function up()
    {
        Schema::table('historicals', function (Blueprint $table) {
            $table->decimal('EBITDA', 8, 2)->after('two_hundred_day_moving_average')->nullable();
            $table->decimal('earnings_per_share_current', 8, 2)->after('EBITDA')->nullable();
            $table->decimal('earnings_per_share_next_year', 8, 2)->after('earnings_per_share_current')->nullable();
            $table->decimal('price_to_earnings', 8, 2)->after('earnings_per_share_next_year')->nullable();
            $table->decimal('price_to_sales', 8, 2)->after('price_to_earnings')->nullable();
            $table->decimal('price_to_book', 8, 2)->after('price_to_sales')->nullable();
            $table->decimal('year_high', 8, 2)->after('price_to_book')->nullable();
            $table->decimal('year_low', 8, 2)->after('year_high')->nullable();
            $table->decimal('market_cap', 8, 2)->after('year_low')->nullable();
            $table->decimal('dividend_yield', 8, 2)->after('market_cap')->nullable();
